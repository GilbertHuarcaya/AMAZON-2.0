...
function generateCartItems(cartItems){
	...
	cartItems.forEach((item)=> {
		...
		<div class="cart-item flex items-center pb-4 border-b border-gray-100">
			<div class="cart-item-image w-40 h-24 bg-white p-4 rounded-lg"><img class="w-full h-full object-contain"  src="${item.image}" alt="laptop"/>
			</div>
			<div class="cart-item-details flex-grow">
			<div   class="    cart-item-details-title    font-bold    text-sm text-gray-600  ">  ${item.name}
			</div>
                	<div class="cart-item-brand text-sm text-gray-400">${item.make}</div></div>
			<div class="cart-item-counter w-48 flex items-center text-gray-400">
			...
			<div data-id="${item.id}" class="cart-item-total-cost w-48 font-bold text-gray-400">
